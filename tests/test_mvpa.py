...
def synthetic_dataset(tmp_path_factory):
    ...
    # The find function expects a session directory
    fprep_dir = tmp_path / "derivatives" / "fmriprep" / f"sub-{sub_id}" / "ses-1" / "func"
    ...
    # The mask path now needs to match what find_fmriprep_files will construct
    mask_path = fprep_dir / f"sub-{sub_id}_ses-1_task-discountFix_run-1_space-MNI_desc-brain_mask.nii.gz"
    nib.save(mask_img, mask_path)

    # --- CRITICAL FIX: Create a dummy BOLD file so find_fmriprep_files can succeed ---
    dummy_bold_img = nib.Nifti1Image(np.zeros(mask_shape + (10,)), mask_affine)
    bold_path = fprep_dir / f"sub-{sub_id}_ses-1_task-discountFix_run-1_space-MNI_desc-preproc_bold.nii.gz"
    nib.save(dummy_bold_img, bold_path)
    
    # --- Create a dummy confounds file ---
    confounds_df = pd.DataFrame({'dummy': np.zeros(10)})
    confounds_path = fprep_dir / f"sub-{sub_id}_ses-1_task-discountFix_run-1_desc-confounds_timeseries.tsv"
        ...
        'analysis_params': {
            'mvpa': {
                ...
                'classification': {
                    ...
                    'scoring': 'accuracy',
                    'cv_folds': 2
                    # random_state cannot be used with StratifiedGroupKFold
                },
                'regression': {
    ...
    return {
        ...
        # Pass the root fmriprep dir, not the subject-specific one
